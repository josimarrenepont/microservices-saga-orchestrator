...
public class SagaExecutionController {
    ...
    private static final String SAGA_LOG_ID = "ORDER ID: %s | TRANSACTION IDD %s | EVENT ID %s";

    public ETopics getNextTopic(Event event){
        if(isEmpty(event.getSource()) || isEmpty(event.getStatus())){
            throw new ValidateException("Source and status nusr be informed.");
        }
    ...
    private void logCurrentSaga(Event event, ETopics topic){
        ...
        switch (event.getStatus()){
            case SUCCESS -> log.info("### CURRENT SAGA: {} | SUCCESS | NEXT TOPIC {} | {}",
                 source, topic, sagaId);
            case ROLLBACK_PENDING -> log.info("### CURRENT SAGA: {} | SENDING TO ROLLBACK " +
                            "CURRENT SERVICE | NEXT TOPIC {} | {}",
                    source, topic, sagaId);
            case FAIL -> log.info("### CURRENT SAGA: {} | SENDING TO ROLLBACK PREVIOUS | NEXT TOPIC {} | {}",
                    source, topic, sagaId);
        }
    }
